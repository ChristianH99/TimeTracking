...
class HoursFormat(models.TextChoices):
    """How a duration is written wherever this app prints one.

    The two are the same number and neither is a rounding of the other: 7,5 h
    and 7:30 h are one duration written for two different readers. A payroll
    export and a contract say "37,5 hours a week"; the person who worked them
    read a clock, and 7:30 is what they wrote on paper before this app existed.

    Per person rather than per organisation because it is a reading habit, not
    a policy — and because the same figures have to reconcile either way, which
    they do: `apps/timesheets/hours.py` formats one stored count of minutes.
    """

    DECIMAL = "decimal", _("decimal — 7,5 h")
    CLOCK = "clock", _("clock — 7:30 h")


class Preferences(models.Model):
    """One person's own settings.

    Per person and not per organisation, because this is about how somebody
    reads a page rather than about what the page says. The roster, the hours
    and the entitlements are shared and agreed; this is not.
    ...
    hours_format = models.CharField(
        _("hours"), max_length=10,
        choices=HoursFormat.choices, default=HoursFormat.DECIMAL,
        help_text=_("How durations are written on every page that shows one."),
    )
